  ...
  form({-name=>"formProfile", -class=>"", -method=>"post", -action000=>$c->url_for("профиль сохранить"), "ng-app"=>"formProfile", "ng-controller"=>"formProfileControll as ctrl",},#  "ng-submit"=>"ctrl.Submit()"
    ...
    div({'ng-if'=>"ctrl.ready", -class=>"row",},
      ...
      div({-class=>"col m4 s12"},
        ...
        ($error || '') && div({-style=>"padding:0.5rem;", -class=>"card red lighten-5 red-text text-darken-4",}, 'Ошибка внешней авторизации: ', $error),
        ($conflict || '') && div({-style=>"padding:0.5rem;", -class=>"card red lighten-5 red-text text-darken-4",}, 
