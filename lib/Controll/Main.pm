...
sub index {
  ...
  return $c->render('main/home',
    #~ handler=>'ep',
    'head-title' => 'Система размещения и поиска предложений и спроса услуг по грузоперевозкам и оказания услуг по работе различной техники',#Грузоперевозки - спецтехника - размещение - поиск - заказ
    'meta-description'=> "Сервис размещения транспортных объявлений, поиск предложений транспортных услуг, поиск спроса услуг по грузоперевозкам, оказание услуг по работе различной техники. Заказчик и исполнитель транспортных услуг находят друг друга.",
    'meta-keywords'=>"сервис грузоперевозок, автотехника, услуги транспорта, найти транспорт, заказать транспорт, разместить объявление, ". join(', ', map $_->{title}, @{$c->top_category}),
